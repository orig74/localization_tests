...
class optical_flow_track(object):
    ...
    def feed(self,frame):
        if self.old_gray is None:
            ...
            self.p0=cv2.goodFeaturesToTrack(self.old_gray,400,0.01,20).reshape(-1,1,2)
            self.start_pts=self.p0.copy().reshape((-1,2))
        ...
        #    self.start_pts=good_old
        #else:
        self.start_pts=self.start_pts[(st==1).flatten()]
        ...
        for i,new in enumerate(good_new):
             a,b = new.ravel()
             cv2.circle(frame,(a,b),2,self.color[i].tolist(),-1)

        # Now update the previous frame and previous points
        self.old_gray = frame_gray
        self.p0 = good_new.reshape(-1,1,2)
        #return frame,good_old,good_new
        return frame,self.start_pts,good_new 
